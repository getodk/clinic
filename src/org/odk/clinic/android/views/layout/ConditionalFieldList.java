package org.odk.clinic.android.views.layout;
...
import org.odk.clinic.android.widgets.AbstractQuestionWidget;
import org.odk.clinic.android.widgets.IMultipartSelectWidget;
import org.odk.clinic.android.widgets.WidgetFactory;
...
public class ConditionalFieldList  implements IGroupLayout {
	...
	public void buildView(GroupView view, List<FormIndex> indices, String instancePath, FormEntryCaption[] groups) {
        ...
        FormEntryModel model = Clinic.getInstance().getFormEntryController().getModel();
		...
		FormEntryPrompt formEntryPrompt = Clinic.getInstance().getFormEntryController().getModel().getQuestionPrompt(fi);
	        ...
	        for ( int i = 0 ; i < values.size(); ++i ) {
	        	...
	        	if ( nestedWidgets != null ) {
		        	for ( FormIndex nestedFormIndex : nestedWidgets ) {
		        		formEntryPrompt = Clinic.getInstance().getFormEntryController().getModel().getQuestionPrompt(nestedFormIndex);
        ...
        for ( ++iResidual ; iResidual < residual.size() ; ++iResidual ) {
    		...
    		formEntryPrompt = Clinic.getInstance().getFormEntryController().getModel().getQuestionPrompt(fi);
            // if question or answer type is not supported, use text widget
