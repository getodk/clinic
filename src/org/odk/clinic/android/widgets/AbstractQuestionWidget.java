package org.odk.clinic.android.widgets;
...
import org.odk.clinic.android.views.IAVTLayout;
import org.odk.clinic.android.widgets.AbstractQuestionWidget.OnDescendantRequestFocusChangeListener.FocusChangeState;
...
public abstract class AbstractQuestionWidget extends LinearLayout implements IBinaryWidget,
    ...
    public final void buildViewEnd(AbstractFolioView fv) {
        updateViewAfterAnswer();
        setEnabled(Clinic.getInstance().getFormEntryController().getModel().isIndexRelevant(prompt.getIndex()));
    	...
    	if ( !prompt.isReadOnly() ) {
			...
			if (evaluateConstraints) {
				...
				if ( saveStatus != FormEntryController.ANSWER_OK ) {
					Clinic.getInstance().createConstraintToast(
	...
	public void setFocus(Context context) {
		...
		Clinic.getInstance().hideSoftKeyboard(this);
    }
